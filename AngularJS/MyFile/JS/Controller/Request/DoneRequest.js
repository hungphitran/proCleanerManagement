function doneRequestCtrl($scope,$location,$http,doneRequestDetailService,$rootScope){
    ...
    $scope.tieuchiSearch = [" Tất Cả "," Mã Số ","Ngày Yêu Cầu", "Số ĐT Khách Hàng", "Loại Yêu cầu", "Chi Phí"];
    ...
    $scope.initTimeRangeLoadDoneRequest = function(startDate, endDate){
        $scope.timePicker = $("#timeRangeLoadDoneRequest").daterangepicker({
            datepickerOptions : {
                numberOfMonths : 2
            },
            presetRanges:[
        ...
        $("#timeRangeLoadDoneRequest").daterangepicker("setRange",defaultTimeRange);
    }

    $scope.checkToLoadDoneRequest = function(){
        var jsonObj = JSON.parse($('#timeRangeLoadDoneRequest').val());
        ...
        $scope.loadDoneRequestData();
    }

    $scope.loadDoneRequestData = function(){
        ...
        $http.post('/api/request/list/doneAndPayment',$scope.timeRange)
    ...
    $scope.search = function(newValue){
        ...
        switch($scope.typeSearch){
            case " Tất Cả ":
                for(var i=0;i<$scope.requestHistoryListOriginal.length;i++)
                {
                    if(($scope.requestHistoryListOriginal[i].id+"").toUpperCase().indexOf(newValue.toUpperCase())>-1 
                        ||($scope.requestHistoryListOriginal[i].sdtkhachhang+"").toUpperCase().indexOf(newValue.toUpperCase())>-1
                        ||($scope.requestHistoryListOriginal[i].loaiyeucau+"").toUpperCase().indexOf(newValue.toUpperCase())>-1
                        ||($scope.requestHistoryListOriginal[i].chiphi+"").toUpperCase().indexOf(newValue.toUpperCase())>-1
                        ||($scope.requestHistoryListOriginal[i].ngaydatyeucau.getUTCDate()+"/"+
    ...
    // $scope.deleteRequest = function(cmnd,hoten){
    //     alert('xoa '+hoten);
    //     $http.delete('api/stuff/' + cmnd)
    //         .success(function(data){
    ...
    $(document).ready(function(){
        ...
        $scope.initTimeRangeLoadDoneRequest($scope.timeRange.startDate, $scope.timeRange.endDate);
        $scope.loadDoneRequestData();
